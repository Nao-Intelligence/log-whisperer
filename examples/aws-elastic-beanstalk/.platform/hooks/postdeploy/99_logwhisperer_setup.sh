...
# Use printf %q to safely escape any special characters (quotes, $, spaces, etc.)
_lw_env_vars=(
  "APP_ROLE=${ROLE}"
...
: > /opt/logwhisperer/env
for _lw_pair in "${_lw_env_vars[@]}"; do
  _lw_key="${_lw_pair%%=*}"
  _lw_val="${_lw_pair#*=}"
  printf '%s=%q\n' "$_lw_key" "$_lw_val" >> /opt/logwhisperer/env
done
unset _lw_env_vars _lw_pair _lw_key _lw_val
